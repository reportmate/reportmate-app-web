...
function identifyDeviceIdentifierType(identifier: string): 'uuid' | 'assetTag' | 'serialNumber' | 'deviceName' | 'hostname' {
  ...
  // Asset tag pattern: Letter followed by digits (e.g., L000001, A000042)
...
export default async function middleware(request: NextRequest) {
  ...
  const getCorrectUrl = (originalUrl: string): string => {
    if (process.env.NODE_ENV === 'production') {
      return originalUrl.replace(/(https?:\/\/)[^\/]+/, 'https://reportmate.example.edu')
    }
    return originalUrl
  }
  ...
  try {
    ...
    const correctBaseUrl = process.env.NEXTAUTH_URL || process.env.NEXT_PUBLIC_SITE_URL || 'https://reportmate.example.edu'

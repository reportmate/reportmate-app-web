...
function HardwarePageContent() {
  ...
  const processedHardware = hardware.map(hardwareRecord => {
    ...
    if (hardwareRecord.serialNumber === 'TESTSERIAL0001') {
  ...
  const targetDevice = hardware.find(h => h.serialNumber === 'TESTSERIAL0001')
  const targetDeviceInMainAPI = devices.find(d => d.serialNumber === 'TESTSERIAL0001')
  if (targetDevice) {
    console.log('[DEVICE FOUND] TESTSERIAL0001 hardware record:', targetDevice)
  } else {
    ...
    console.log('[DEVICE NOT FOUND] TESTSERIAL0001 not in hardware array.');
  ...
  if (targetDeviceInMainAPI) {
    console.log('[MAIN API] TESTSERIAL0001 found in devices API:', {
    ...
    console.log('[MAIN API] TESTSERIAL0001 not found in devices API either.');
                    ...
                    <tr key={hw.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      ...
                      <td className="px-6 py-4 text-xs text-gray-900 dark:text-white">
                        {(() => {
                          ...
                          if (process.env.NODE_ENV === 'development' && hw.serialNumber === 'TESTSERIAL0001') {

...
export async function GET(request: Request) {
  ...
  try {
    ...
    const detectPlatform = (serialNumber: string): string => {
      ...
      if (serialNumber.match(/^[0-9][A-Z0-9]{11,13}$/)) return 'Windows' // TESTSERIAL0001
      if (serialNumber.match(/^WIN-/)) return 'Windows'                  // WIN-prefixed
      if (serialNumber.match(/^[A-Z0-9]{7,8}$/)) return 'Windows'        // PF4QEAX7, DUMMY02, DUMMY04 etc.
      if (serialNumber.match(/^[A-Z]{2}[0-9][A-Z0-9]{4}$/)) return 'Windows' // DUMMY13
      if (serialNumber.match(/^[0-9]{2}[A-Z0-9]{5}$/)) return 'Windows'  // DUMMY05, DUMMY01
      ...
      if (serialNumber.match(/^[8-9][A-Z0-9]{6}$/)) return 'macOS'       // DUMMY22 (true Apple serials)
      if (serialNumber.match(/^[CDF][A-Z0-9]{9}$/)) return 'macOS'       // DUMMY17, DUMMY20 (true Apple serials)

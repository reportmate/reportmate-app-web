...
export async function GET(request: Request) {
  try {
    ...
    // Call the dedicated FastAPI /api/v1/security endpoint (NOT /api/v1/devices, NOT the device list)
    // This endpoint returns optimized security summary data, not full device modules
    try {
      const url = `${apiBaseUrl}/api/v1/security${includeArchived ? '?includeArchived=true' : ''}`
      ...
      // Data is already formatted by FastAPI /api/v1/security endpoint

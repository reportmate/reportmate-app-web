 ...
 * 
 * OPTIMIZED: Uses bulk /api/v1/installs endpoint instead of 
 ...
 * Uses the optimized /api/v1/installs endpoint instead of individual device fetches
 */
export async function fetchBulkInstallRecords(): Promise<any[]> {
  ...
  // Use the bulk /api/v1/installs endpoint - much faster than individual fetches!
  const response = await fetch(`${API_BASE_URL}/api/v1/installs`, {
    cache: 'no-store',
    headers
  });

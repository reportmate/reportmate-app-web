...
export function GraphicsDistributionChart({ 
  ...
  }, [devices, globalSelectedPlatforms, globalSelectedModels, globalSelectedArchitectures, globalSelectedDeviceTypes, globalSelectedMemoryRanges, globalSelectedStorageRanges, globalSelectedProcessors, selectedGraphics, sortBy])
  ...
  return (
    <div className={`bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 ${className}`}>
      <div className="flex items-center justify-between mb-2">
        <h3 className="text-sm font-semibold text-gray-900 dark:text-white cursor-pointer hover:text-blue-600 dark:hover:text-blue-400 select-none transition-colors" onClick={() => setSortBy(s => s === 'count' ? 'alpha' : 'count')} title={sortBy === 'count' ? 'Sort alphabetically' : 'Sort by quantity'}>Graphics {sortBy === 'alpha' ? '(A-Z)' : ''}</h3>
      </div>

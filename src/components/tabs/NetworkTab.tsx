    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="lg:w-[30%] space-y-4">
          {/* VPN Configurations - shows all configured VPNs, connected ones highlighted */}
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
              <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold text-sm text-gray-900 dark:text-white">VPN</span>
                </div>
                <div className="flex items-center gap-2">
                  {hasConnectedVpn && (
                    <span className="text-xs font-medium px-1.5 py-0.5 rounded-full bg-purple-100 dark:bg-purple-900 text-purple-700 dark:text-purple-300">
                      {connectedVpns.length} connected
                    </span>
                  )}
                  <span className="text-xs text-gray-500 dark:text-gray-400">{networkData.vpnConnections.length}</span>
                </div>
              </div>
              <ul className="divide-y divide-gray-100 dark:divide-gray-700">
                {networkData.vpnConnections.map((vpn: any, index: number) => {
                  const isConnected = vpn.status === 'Connected' || vpn.status === 'connected'
                  return (
                    <li key={vpn.name || index} className="px-4 py-2.5 flex items-center justify-between">
                      <div className="min-w-0">
                        <div className="text-sm font-medium text-gray-900 dark:text-white truncate">{vpn.name}</div>
                        {vpn.type && vpn.type !== 'VPN' && (
                          <div className="text-xs text-gray-500 dark:text-gray-400">{vpn.type}</div>
                        )}
                        {vpn.serverAddress && (
                          <div className="text-xs text-gray-400 dark:text-gray-500 font-mono truncate">{vpn.serverAddress}</div>
                        )}
                      </div>
                      <span className={`ml-2 flex-shrink-0 text-xs font-medium px-1.5 py-0.5 rounded-full ${
                        isConnected
      ...
      <DebugAccordion
        data={device?.modules?.network}
        label="device.modules.network"
        moduleVersion={device?.modules?.network?.moduleVersion}
      />
    </div>
  )
}

...
async function testDeviceQuery() {
  ...
  try {
    ...
    const result = await pool.query(deviceQuery, ['TESTSERIAL0001']);
    ...
    const idResult = await pool.query('SELECT * FROM devices WHERE id = $1', ['TESTSERIAL0001']);
    console.log('Query by id:', idResult.rows.length, 'rows');
    
    const serialResult = await pool.query('SELECT * FROM devices WHERE serial_number = $1', ['TESTSERIAL0001']);

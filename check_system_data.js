...
client.connect().then(() => {
    ...
    return client.query('SELECT id FROM devices WHERE serial_number = $1', ['TESTSERIAL0001']);
}).then((result) => {
    ...
    if (result.rows.length > 0) {
        console.log('System data for device TESTSERIAL0001:');
        ...
        console.log('No system data found for device TESTSERIAL0001');
    }
    return client.end();
}).catch(err => {
